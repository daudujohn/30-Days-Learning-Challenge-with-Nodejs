...
let readstream = fs.createReadStream(path.resolve('largefile.txt'))
let writestream = fs.createWriteStream(path.resolve('largeFileDuplicate.txt'))
...
readstream.on('end', () => {
    writestream.close()
    console.log('Done duplicating largeFile.txt into largeFileDuplicate.txt')
})
